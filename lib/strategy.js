...
function JwtStrategy(options, verify) {
    ...
    if (options.challenges) {
        var challenges = typeof options.challenges === 'object' ? options.challenges : {}
        this.challenges = {};
        this.challenges.invalidRequest = challenges.invalidRequest || function (req) {
            ...
            return 'Bearer error="invalid_request" error_description="' + error_description + '"';
        };
        this.challenges.invalidToken = challenges.invalidToken || function (req, jwt_err) {
...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    if (!token) {
        if (self.challenges) {
            return self.fail(self.challenges.invalidRequest(req), 400);
        } else {
            // an error used to be passed here but it was ignored by passport
            return self.fail();
        }
    }
    ...
    this._secretOrKeyProvider(req, token, function(secretOrKeyError, secretOrKey) {
        if (secretOrKeyError) {
            ...
            JwtStrategy.JwtVerifier(token, secretOrKey, self._verifOpts, function(jwt_err, payload) {
                if (jwt_err) {
                    if (self.challenges) {
                        return self.fail(self.challenges.invalidToken(req, jwt_err), 401);
                    } else {
                        // leave the error out because passport would just ignore it
                        return self.fail();
                    }
                } else {
