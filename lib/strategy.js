...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    function secretOrKeyProviderDone(secretOrKeyError, secretOrKey) {
        if (secretOrKeyError) {
            ...
            JwtStrategy.JwtVerifier(token, secretOrKey, self._verifOpts, function(jwt_err, payload) {
                if (jwt_err) {
                    ...
                    verified.error = self.error.bind(self);
                    verified.fail = self.fail.bind(self);
                    verified.success = self.success.bind(self);
    ...
    secretOrKeyProviderDone.error = self.error.bind(self);
    secretOrKeyProviderDone.fail = self.fail.bind(self);
    secretOrKeyProviderDone.success = function(secretOrKey) {
        return secretOrKeyProviderDone(null, secretOrKey);
    };
    this._secretOrKeyProvider(req, token, secretOrKeyProviderDone);
};
